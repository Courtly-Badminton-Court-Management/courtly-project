...
export default function WalkinConfirmedModal({
  ...
  useEffect(() => {
    ...
    // Prefetch manager pages
    router.prefetch("/manager/dashboard");
    router.prefetch("/manager/log");
        ...
        >
          ...
          >
            ...
            <p className="mb-4 text-sm text-neutral-600">
              {bookingNos.length > 1
                ? "These walk-in bookings have been recorded in the manager log."
                : "This walk-in booking has been recorded in the manager log."}
            </p>
            ...
            {bookingNos?.length > 0 && (
              ...
              >
                <p className="text-sm font-semibold text-onyx">
                  Booking ID:&nbsp;
                  <CopyToClipboard text={bookingNos} />
                </p>
              </motion.div>
            )}

            {/* ปุ่ม actions (Manager version) */}
            <div className="flex flex-col sm:flex-row justify-center items-stretch gap-2 mt-6">
              <Link
                href="/dashboard"
                className="rounded-md bg-teal-700 px-4 py-2 text-[15px] font-semibold text-white hover:bg-teal-800 transition-all text-center"
              >
                Go to Dashboard
              </Link>
              <Link
                href="/log"
                className="rounded-md border border-teal-200 bg-white px-4 py-2 text-[15px] font-semibold text-teal-700 hover:bg-teal-50 transition-all text-center"
              >
                View Manager Log
              </Link>
              <button
