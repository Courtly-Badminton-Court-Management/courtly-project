...
function LoginContent() {
  ...
  const loginMutation = useAuthLoginCreate({
    mutation: {
      onSuccess: async (payload: any) => {
        ...
        // 1) เก็บ token ใน memory + sessionStorage
        setTokens({ access, refresh: refresh ?? null }, true);

        // ✅ 2) เก็บ token ลง localStorage ด้วย
        localStorage.setItem("accessToken", access);
        if (refresh) {
          localStorage.setItem("refreshToken", refresh);
        }

        // 3) หา role จาก access token
        let role = extractRoleFromAccess(access) as Role | null;
        if (!role) {
          try {
  ...
  return (
    <SplitPage heroPosition="left" formSide={Right} heroSide={<AuthHero side="right" />} />
  );
}
